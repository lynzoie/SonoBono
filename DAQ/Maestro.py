...
def log(*msgline):
    return
    for msg in msgline:
        if 'Not initialized' in msg: return
        print(msg),
    print


class Device(object):
    def __init__(self, con_port="/dev/ttyACM1", ser_port="/dev/ttyACM0", timeout=1):
        # /dev/ttyACM0  and   /dev/ttyACM1  for Linux
        ############################
        ...
        if not type(con_port) == str: self.isInitialized = (self.ser != None)
        if self.isInitialized:
            err_flags = self.get_errors()
            err_flags = str(err_flags)
            log("Device error flags read (" + err_flags + ") and cleared")
        log("Device initialized: " + str(self.isInitialized))
