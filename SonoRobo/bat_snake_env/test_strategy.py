...
class BatSnake_base(py_environment.PyEnvironment):
    def __init__(self, preset=1, 
        ...
        self._action_spec = array_spec.BoundedArraySpec(
            shape=(), dtype=np.int16, minimum=0, maximum=len(self.act.decode)-1, name='action')
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(100,), dtype=np.float32, minimum=0, name='observation')
        self._state = self.echo._echo
        self.reward_setting = {'food': 1, 'step': [0.1, 0.8], 'collide': -2}
    ...
    def _reset(self):
        ...
        self.level = 0
        self.obj.set_new_food(self.level)
        return ts.restart(np.array([self._state], dtype=np.float32).reshape(100,))
    ...
    def _step(self, action):
        ...
        if self._episode_ended:
            return ts.termination(np.array([self._state], dtype=np.float32).reshape(100,), reward=np.round(reward,4))
        else:
            return ts.transition(np.array([self._state], dtype=np.float32).reshape(100,), reward=np.round(reward,4), discount=1.0)
